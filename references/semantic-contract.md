...
- `Module.monomer_pairings` have priority and are parsed as exact substrate-to-monomer evidence. Raw evidence (`monomer_calls`) retains every antiSMASH call without deduplication.
- Pairing multiplicity within one `aSModule` does not by itself imply multiple incorporated residues. Each module contributes at most one default collinear incorporation slot (`incorporation_call`).
- Identical repeated pairings within one module are retained as raw evidence but collapse to one default collinear incorporation slot (`pairing_status="identical_duplicate"`). Unresolved or malformed pairings compare exact raw qualifier text, ensuring distinct malformed entries remain `conflicting` without false duplicate categorization.
- Distinct pairings within one module represent conflicting/alternative evidence and produce one unresolved incorporation slot (`pairing_status="conflicting"`), with raw alternatives retained.
- Identical monomers belonging to distinct `aSModule` objects remain distinct incorporation slots.
- Duplicate pairings on a multi-CDS module generate structural-integrity review signals (`duplicate_monomer_pairing`, `cross_cds_duplicate_monomer_pairing`) but do not prove fragmentation, pseudogenization, frameshift, or assembly error.
...
- The upstream `iterative` flag is preserved as module evidence but does not specify repeat count and is not treated as a general proof or disproof of iterative NRPS chemistry.
- Assembly-line mass values are separate modeled candidates, not fields on the parsed evidence objects. The dedicated `antismash-review-assemblyline` JSON schema is version `0.3.0` after the module-level incorporation interpretation gate.
- Core-mass candidates are formula-derived only for fully resolved, high-confidence proteinogenic amino-acid-like NRPS chains using genuine free amino-acid formulas with peptide dehydration on interpreted incorporation slots. Modules marked incomplete by antiSMASH (`complete=False`), modules with unresolved or ambiguous domain references, unknown `X`, non-proteinogenic calls, conflicting module calls, PKS modules, starter/acyl uncertainty, iterative modules, low-confidence specificity fallbacks, and tailoring chemistry make the full-core candidates null.
- When chemistry is complete enough, linear and head-to-tail cyclic candidate values are emitted separately with `topology_assumption="unknown"`; neither value is the measured final metabolite mass. A head-to-tail cyclic candidate requires at least two incorporated monomers (`total >= 2`); single-residue chains return `null` for the cyclic candidate.
...
- **Mass candidate versioning notice**: Assembly-line core-mass candidates produced by package version `0.1.0` contained a defect where amino-acid residue formulas were mislabeled as free monomers and subsequently dehydrated during polymerization, resulting in masses that were too low by exactly `n × 18.01056468403 Da` for `n` incorporated amino acids. Any mass tables or candidates generated with `v0.1.0` must be regenerated using `v0.2.0+` (`parser_version >= "0.2.0"`).
